...
final class AiVisibilityService
{
    public function __construct(
        ...
        private readonly OpenAiWebSearchClient $openAiWebSearchClient,
     ...
     * @param  array<string,mixed>  $options
     */
    public function runOpenAiWebSearch(AiSourceProvider $provider, string $keyword, array $options = []): AiVisibilityRun
    {
        $keyword = $this->normalizeKeyword($keyword);
        $run = $this->createRun([
            'keyword' => $keyword,
            'prompt' => (string) ($options['prompt'] ?? $keyword),
            'provider_type' => AiVisibilityRun::PROVIDER_OPENAI_WEB_SEARCH,
            'provider_key' => (string) ($provider->provider_key ?? AiSourceProvider::PROVIDER_OPENAI_WEB_SEARCH),
            'ai_source_provider_id' => (int) $provider->id,
            'locale' => (string) ($options['locale'] ?? 'en_US'),
        ]);
        ...
        $reservation = null;
        try {
            $this->assertSourceProviderEnabled($provider, 'OpenAI 信源供应商');
            $reservation = $this->usageQuota->reserveProvider($provider);
            if ($reservation === null) {
                throw new RuntimeException('ai_source_provider_quota_exhausted');
            }
            $result = $this->openAiWebSearchClient->search(
                $provider,
                $keyword,
                array_replace($provider->visibilitySearchOptions(), $options),
            );
